...
异步连接池 + 声明式事务，基于 `asyncpg` 原生连接池与 `contextvars`。
...
pool = ConnectionPool(dsn="postgresql://user:pass@localhost:5432/mydb")
set_default_pool(pool)

@transaction()
async def create_user(name: str, conn=None):
    await conn.execute("INSERT INTO users (name) VALUES ($1)", name)
```

`@transaction()` 需带括号调用；函数最后一个参数名必须为 `conn`（禁止手动传递，由 ContextVar 自动注入事务连接）。
...
```python
@transaction()
async def create_user_with_log(name: str, conn=None):
    await create_user(name)  # 内层通过 SAVEPOINT 接入外层事务
    await conn.execute("INSERT INTO logs (msg) VALUES ($1)", f"created {name}")
```

## 查询

`asyncpg` 占位符为 `$1, $2, ...`，查询返回 `Record` 对象（可按列名下标访问）：
